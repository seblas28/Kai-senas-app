...
export const getSignsByCategory = (category: string): SignInfo[] => {
  ...
  if (category === 'numeros') {
    signs.sort((a, b) => parseInt(a.label) - parseInt(b.label));
  }
...
export const getSignsForTraining = (category: string): SignInfo[] => {
  ...
  if (category === 'abecedario') {
    signs = allSignData.filter(sign => 
      sign.categories.includes('abecedario') && !sign.categories.includes('vocales')
    );
  } else {
    signs = allSignData.filter(sign => sign.categories.includes(category as any));
  }
